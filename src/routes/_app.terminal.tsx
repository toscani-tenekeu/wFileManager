  ...
  if (!user?.isAdmin) {
    return (
      <Alert variant="destructive" className="m-4">
        ...
        <AlertDescription>The terminal is restricted to administrators.</AlertDescription>
      </Alert>
    );
  }
        ...
        <Card className="w-full">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <KeyRound className="h-5 w-5" /> Unlock administrator terminal
            </CardTitle>
            <CardDescription>
              The terminal opens a root shell. Enter the password of the currently signed-in
              wFileManager administrator. The password is verified and is not stored by the
              terminal.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            >
              <Alert variant="destructive">
                <ShieldAlert className="h-4 w-4" />
                <AlertDescription>
                  Root commands directly affect the entire server and cannot be undone by
                  wFileManager.
                </AlertDescription>
              </Alert>
              <div className="grid gap-1.5">
                <Label htmlFor="terminal-password">Current administrator password</Label>
                <Input
              ...
              <Button type="submit" disabled={!password}>
                <KeyRound className="mr-2 h-4 w-4" /> Verify and open terminal
              </Button>
            </form>
          </CardContent>
        </Card>
  ...
  return (
    ...
    >
      ...
      <div className="relative min-h-0 flex-1 bg-[#171719]">
        {status === "connecting" && (
          <div className="pointer-events-none absolute inset-0 z-10 grid place-items-center bg-[#171719]/70 text-sm text-white">
      ...
      <div className="flex items-center justify-between border-t border-border bg-surface/60 px-3 py-1.5 text-[11px] text-muted-foreground">
        <span>
          Interactive PTY · <span className="font-mono">/bin/bash --login</span> · application
          administrator <span className="font-mono">{user.username}</span>
        </span>
