      ...
      <div className="grid gap-4">
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-1.5">
              ...
              <Input value={profile.displayName} disabled={profileLoading} onChange={(event) => setProfile({ ...profile, displayName: event.target.value })} />
            </div>
            <div className="grid gap-1.5">
            ...
            <div className="grid gap-1.5">
              <Label>Email</Label>
              <Input type="email" value={profile.email} disabled={profileLoading} onChange={(event) => setProfile({ ...profile, email: event.target.value })} />
            </div>
            <div className="grid gap-1.5">
              <Label>Timezone</Label>
              <Select value={profile.timezone} disabled={profileLoading} onValueChange={(timezone) => setProfile({ ...profile, timezone })}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Changing it also updates your dedicated Linux sudo account.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="sm:col-span-3 flex justify-end">
              <Button
                disabled={passwordSaving || password.current.length < 1 || password.next.length < 8 || password.next !== password.confirm}
                onClick={async () => {
                  setPasswordSaving(true);
                  try {
                    await wfilemanagerApi.changePassword(password.current, password.next);
                    try {
                      await localApi.syncLinuxPassword(password.next);
                    } catch (linuxError) {
                      toast.warning(linuxError instanceof Error ? `Application password changed, but Linux password synchronization failed: ${linuxError.message}` : "Application password changed, but Linux password synchronization failed");
                    }
              ...
              >
                {passwordSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Change password
              </Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <div>
                <CardTitle className="flex items-center gap-2 text-base"><MonitorSmartphone className="h-4 w-4" /> Active sessions</CardTitle>
                <CardDescription>Devices currently authenticated with your account.</CardDescription>
              </div>
              <Button size="icon" variant="outline" onClick={() => void loadSessions()} aria-label="Refresh sessions"><RefreshCw className={`h-4 w-4 ${sessionsLoading ? "animate-spin" : ""}`} /></Button>
            </div>
          </CardHeader>
          <CardContent>
            {sessionsLoading ? (
                    ...
                    {!session.current && (
                      <Button size="sm" variant="outline" onClick={async () => {
                        try {
                          ...
                          toast.error(value instanceof Error ? value.message : "Unable to revoke session");
                        }
                      }}>Revoke</Button>
                    )}
                  </li>
            ...
            <div className="mt-4 flex justify-end">
              <Button variant="destructive" onClick={async () => {
                try {
                  ...
                  toast.error(value instanceof Error ? value.message : "Unable to revoke sessions");
                }
