...
const GROUPS: Array<{ name: string; permissions: PermissionKey[] }> = [
  ...
  { name: "Administration", permissions: ["use_terminal", "manage_users", "manage_roles"] },
];
...
function Roles() {
  ...
  const administratorLocked = active?.isSystem && active.name === "Administrator";
  ...
  const load = async () => {
    ...
    try {
      ...
      const selected = result.roles.find((role) => role.id === activeId) || result.roles[0] || null;
  ...
  const create = async () => {
    setSaving(true);
    try {
      const result = await wfilemanagerApi.createRole(newRole);
  ...
  const remove = async () => {
    ...
    try {
      ...
      const next = roles.filter((role) => role.id !== active.id);
      setRoles(next);
      setActiveId(next[0]?.id || null);
      if (next[0]) setDraft(roleDraft(next[0]));
      setDeleteOpen(false);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl p-6">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Roles are stored in Supabase and can be assigned to wFileManager users.</p>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
        <Card>
          ...
          <CardContent className="p-0">
            <ScrollArea className="max-h-[650px]">
              {loading ? <div className="flex items-center justify-center gap-2 p-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading roles…</div> : (
                <ul className="divide-y divide-border">
                  {roles.map((role) => (
                    <li key={role.id}>
                      <button
                        onClick={() => setActiveId(role.id)}
                        className={cn("flex w-full items-center gap-2 px-4 py-3 text-left hover:bg-muted/50", activeId === role.id && "bg-muted")}
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 text-sm font-medium">
                            <span className="truncate">{role.name}</span>
                            {role.isSystem && <Badge variant="outline" className="h-4 px-1 text-[10px]">system</Badge>}
                          </div>
        ...
        <Card>
          {!active ? <CardContent className="grid min-h-[480px] place-items-center text-sm text-muted-foreground">Select or create a role.</CardContent> : (
            <>
              <CardHeader>
              ...
              <CardContent className="space-y-6">
                {administratorLocked && <Alert><AlertDescription>The Administrator role always keeps every permission and cannot be reduced.</AlertDescription></Alert>}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Permission matrix</p><p className="mt-1 text-xs text-muted-foreground">These permissions are now returned with each session. Path restrictions remain a later step.</p></div>
                    ...
                    <section key={group.name}>
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        {group.permissions.map((permission) => {
                          ...
                          return (
                            <label key={permission} className={cn("flex cursor-pointer items-center gap-2 rounded-md border border-border px-3 py-2 text-sm hover:bg-muted/50", checked && "border-primary/40 bg-primary/5", administratorLocked && "cursor-not-allowed opacity-80")}>
                              <Checkbox checked={checked} disabled={administratorLocked} onCheckedChange={() => toggle(permission)} />
      ...
      <Dialog open={newOpen} onOpenChange={setNewOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Create role</DialogTitle><DialogDescription>Create a reusable role, then assign it when adding users.</DialogDescription></DialogHeader>
          <div className="grid gap-4">
            ...
            <div className="grid gap-2"><Label>Initial permissions</Label><div className="grid max-h-64 gap-1 overflow-auto rounded-md border border-border p-2 sm:grid-cols-2">{PERMISSION_KEYS.map((permission) => <label key={permission} className="flex items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-muted"><Checkbox checked={newRole.permissions.includes(permission)} onCheckedChange={() => setNewRole((current) => ({ ...current, permissions: current.permissions.includes(permission) ? current.permissions.filter((value) => value !== permission) : [...current.permissions, permission] }))} /><span className="capitalize">{labelize(permission)}</span></label>)}</div></div>
          </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete {active?.name}?</AlertDialogTitle><AlertDialogDescription>This removes the role permanently. Roles assigned to users cannot be deleted.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => void remove()}>Delete role</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
    </div>
  );
}
