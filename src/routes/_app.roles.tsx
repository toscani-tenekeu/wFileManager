...
const GROUPS: Array<{ name: string; permissions: PermissionKey[] }> = [
  {
    ...
    permissions: ["browse", "view", "preview", "read", "download", "calculate_checksums"],
  },
  {
    name: "Create and modify",
    permissions: ["create_files", "create_directories", "edit", "rename", "copy", "move", "upload"],
  },
  {
...
function Roles() {
  ...
  const [newRole, setNewRole] = useState({
    name: "",
    description: "",
    permissions: ["browse", "view", "read"] as string[],
  });
  ...
  const permissionGrid = (permissions: string[], target: "draft" | "new", disabled = false) => (
              ...
              return (
                ...
                >
                  <Checkbox
                    checked={checked}
                    disabled={disabled}
                    onCheckedChange={() => toggle(permission, target)}
                  />
  ...
  return (
    <div className="mx-auto w-full max-w-6xl p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
                  ...
                  {roles.map((role) => (
                    <li key={role.id}>
                      <button
                        ...
                        className={cn(
                          "flex w-full items-center gap-2 px-4 py-3 text-left hover:bg-muted/50",
                          activeId === role.id && "bg-muted",
                        )}
                      >
        ...
        <Card>
          {!active ? (
            <CardContent className="grid min-h-[480px] place-items-center text-sm text-muted-foreground">
