...
function sanitizeResult(value: unknown): unknown {
  ...
  if (Array.isArray(result.users))
    return {
      ...result,
      users: result.users.map((user) => sanitizeUser(user as { permissions?: unknown })),
    };
  if (result.user && typeof result.user === "object")
    return { ...result, user: sanitizeUser(result.user as { permissions?: unknown }) };
      ...
      GET: async ({ request }) => {
        try {
          ...
          if (scope === "roles" && action === "permissions")
            return json(sanitizeResult(rolePermissions(user)));
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (scope === "auth" && action === "users")
            return json(sanitizeResult(createUser(user, payload)), 201);
