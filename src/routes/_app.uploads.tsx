  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(value))
        setRecent(value.filter((item): item is string => typeof item === "string").slice(0, 5));
    } catch {
      setRecent([]);
    }
  }, []);
  ...
  const upload = async (files: FileList | File[]) => {
    ...
    try {
      ...
      if (isAbortError(cause)) {
        setResults(
          values.map((file) => ({ name: file.name, size: file.size, status: "cancelled" })),
        );
    ...
    <div className="mx-auto w-full max-w-4xl p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Choose a server folder visually, then upload files with progress and cancellation.
        </p>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {QUICK_DESTINATIONS.map((path) => (
              <Button
                ...
                type="button"
                size="sm"
                variant={destination === path ? "secondary" : "outline"}
          ...
          >
            <div>
              ...
              <div className="mt-4 flex justify-center">
                ...
                >
                  {uploading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Choose files
                </Button>
              </div>
            </div>
          </div>
      ...
      <Dialog open={pickerOpen} onOpenChange={setPickerOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Select upload destination</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-md border border-border">
              <ScrollArea className="h-80">
                {pickerLoading ? (
                  ...
                  <div className="grid h-80 place-items-center text-sm text-muted-foreground">
                    No visible subdirectories in this location.
                  </div>
                ) : (
                  <div className="grid gap-1 p-2 sm:grid-cols-2">
                    {directories.map((entry) => (
                      <button
                        key={entry.path}
                        type="button"
                        className="flex items-center gap-3 rounded-md border border-transparent px-3 py-2 text-left hover:border-border hover:bg-muted/60"
                        onClick={() => void loadPicker(entry.path)}
                      >
            ...
            <div className="rounded-md bg-muted/50 px-3 py-2 text-xs">
              Selected folder: <span className="font-mono">{pickerPath}</span>
            </div>
          </div>
          <DialogFooter>
