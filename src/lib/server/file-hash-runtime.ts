...
export async function fileIntegrityHash(inputPath: unknown, algorithmInput: unknown) {
  ...
  if (!info.isFile()) throw new LocalApiError(400, "Integrity hashes are available only for regular files");
  const algorithm = String(algorithmInput || "sha256").toLowerCase();
  if (algorithm !== "sha256" && algorithm !== "sha512") throw new LocalApiError(400, "Unsupported integrity algorithm");
