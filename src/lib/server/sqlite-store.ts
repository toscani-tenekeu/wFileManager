...
function meta(key: string) {
  const row = db().prepare("SELECT value FROM wfm_meta WHERE key = ?").get(key) as
...
function audit(user: UserRow | null, action: string, target?: string, result = "success") {
  db()
    .prepare(
      "INSERT INTO wfm_audit_logs(user_id, username, action, target, result, created_at) VALUES(?, ?, ?, ?, ?, ?)",
    )
...
function permissionsFor(user: UserRow) {
  if (user.is_admin) return [...PERMISSIONS];
...
function assertPermission(user: UserRow, permission: string) {
  ...
  if (!permissionsFor(user).includes(permission))
    throw new SqliteAuthError(403, `Missing ${permission.replace(/_/g, " ")} permission.`);
}
...
function ensureSystemRoles() {
  ...
  const roles = [
    ...
    [
      "role-file-manager",
      "File Manager",
      "Manage files without user and role administration.",
      PERMISSIONS.filter((p) => !["manage_users", "manage_roles"].includes(p)),
      1,
    ],
    [
      "role-read-only",
      ...
      ["browse", "view", "preview", "read", "download"],
      1,
    ],
  ] as const;
  const statement = db().prepare(
    "INSERT OR IGNORE INTO wfm_roles(id, name, description, permissions, is_system, created_at, updated_at) VALUES(?, ?, ?, ?, ?, ?, ?)",
  );
...
export function setup(data: Record<string, unknown>) {
  ...
  if (displayName.length < 2)
    throw new SqliteAuthError(400, "Display name must contain at least 2 characters.");
...
export function login(data: Record<string, unknown>, request?: Request) {
  ...
  const user = db()
    .prepare(
      "SELECT * FROM wfm_users WHERE username = ? COLLATE NOCASE OR email = ? COLLATE NOCASE LIMIT 1",
    )
...
export function sessionUser(token: string, touch = true) {
  ...
  if (touch)
    db()
      .prepare("UPDATE wfm_sessions SET last_seen_at = ? WHERE token_hash = ?")
...
export function verifyPassword(user: UserRow, password: unknown) {
  ...
  if (!safeEqual(passwordHash(value, user.password_salt), user.password_hash))
...
export function listUsers(actor: UserRow) {
  ...
  const rows = db()
    .prepare("SELECT * FROM wfm_users ORDER BY is_admin DESC, username COLLATE NOCASE")
    .all() as UserRow[];
...
export function createUser(actor: UserRow, data: Record<string, unknown>) {
  ...
  const id = randomUUID();
  const timestamp = now();
  const status = ["active", "disabled", "invited"].includes(String(data.status))
    ? String(data.status)
    : "active";
  ...
  try {
    ...
    throw new SqliteAuthError(
      409,
      error instanceof Error ? error.message : "Username or email already exists.",
    );
  }
  const user = getUserById(id)!;
...
function publicRole(row: Row) {
  const members = db()
    .prepare("SELECT COUNT(*) AS count FROM wfm_users WHERE role_id = ?")
...
export function rolePermissions(user: UserRow) {
  ...
  return {
    roleId: user.role_id,
    roleName: user.is_admin ? "Administrator" : role ? String(role.name) : null,
    permissions: permissionsFor(user),
  };
}
...
export function listRoles(user: UserRow) {
  ...
  const rows = db()
    .prepare("SELECT * FROM wfm_roles ORDER BY is_system DESC, name COLLATE NOCASE")
    .all() as Row[];
...
function normalizedPermissions(input: unknown) {
  ...
  return [
    ...new Set(
      input.map(String).filter((item) => (PERMISSIONS as readonly string[]).includes(item)),
    ),
  ];
}
...
export function createRole(user: UserRow, data: Record<string, unknown>) {
  ...
  if (name.length < 2)
    throw new SqliteAuthError(400, "Role name must contain at least 2 characters.");
  const id = randomUUID();
  const timestamp = now();
  db()
    .prepare(
      "INSERT INTO wfm_roles(id, name, description, permissions, is_system, created_at, updated_at) VALUES(?, ?, ?, ?, 0, ?, ?)",
    )
    .run(
      id,
      name,
      String(data.description || ""),
      JSON.stringify(normalizedPermissions(data.permissions)),
      timestamp,
      timestamp,
    );
...
export function updateRole(user: UserRow, data: Record<string, unknown>) {
  ...
  db()
    .prepare(
      "UPDATE wfm_roles SET name = ?, description = ?, permissions = ?, updated_at = ? WHERE id = ?",
    )
...
export function deleteRole(user: UserRow, idInput: unknown) {
  ...
  if (members.count > 0)
    throw new SqliteAuthError(409, "Move users to another role before deleting this role.");
...
export function updateProfile(user: UserRow, data: Record<string, unknown>) {
  ...
  db()
    .prepare(
      "UPDATE wfm_users SET display_name = ?, email = ?, timezone = ?, updated_at = ? WHERE id = ?",
    )
...
export function changePassword(user: UserRow, data: Record<string, unknown>, currentToken: string) {
  ...
  db()
    .prepare(
      "UPDATE wfm_users SET password_hash = ?, password_salt = ?, must_change_password = 0, updated_at = ? WHERE id = ?",
    )
    .run(credential.hash, credential.salt, now(), user.id);
  db()
    .prepare("DELETE FROM wfm_sessions WHERE user_id = ? AND token_hash <> ?")
    .run(user.id, tokenHash(currentToken));
...
export function revokeSessions(user: UserRow, data: Record<string, unknown>, currentToken: string) {
  ...
  const id = String(data.id || "");
  const row = db()
    .prepare("SELECT token_hash FROM wfm_sessions WHERE id = ? AND user_id = ?")
    .get(id, user.id) as { token_hash?: string } | undefined;
...
export function createNotification(user: UserRow, data: Record<string, unknown>) {
  ...
  const id = randomUUID();
  const createdAt = now();
  const tone = ["info", "success", "warning", "error"].includes(String(data.tone))
    ? String(data.tone)
    : "info";
  db()
    .prepare(
      "INSERT INTO wfm_notifications(id, user_id, title, message, tone, link, source, read_at, created_at, expires_at) VALUES(?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)",
    )
    .run(
      id,
      user.id,
      ...
      String(data.message || ""),
      tone,
      data.link ? String(data.link) : null,
  ...
  return {
    notification: publicNotification(
      db().prepare("SELECT * FROM wfm_notifications WHERE id = ?").get(id) as Row,
    ),
  };
}
...
export function updateNotifications(user: UserRow, data: Record<string, unknown>) {
  ...
  if (data.markAll)
    db().prepare("UPDATE wfm_notifications SET read_at = ? WHERE user_id = ?").run(readAt, user.id);
  else
    db()
      .prepare("UPDATE wfm_notifications SET read_at = ? WHERE id = ? AND user_id = ?")
      .run(readAt, String(data.id || ""), user.id);
  return { success: true as const };
}
...
export function deleteNotifications(user: UserRow, data: Record<string, unknown>) {
  if (data.all) db().prepare("DELETE FROM wfm_notifications WHERE user_id = ?").run(user.id);
    ...
    db()
      .prepare("DELETE FROM wfm_notifications WHERE id = ? AND user_id = ?")
      .run(String(data.id || ""), user.id);
  return { success: true as const };
}
...
export function presence() {
  ...
  const row = db()
    .prepare(
      "SELECT COUNT(DISTINCT user_id) AS count FROM wfm_sessions WHERE last_seen_at >= ? AND expires_at > ?",
    )
    .get(threshold, now()) as { count: number };
  return {
    onlineUsers: Number(row.count || 0),
...
export function rootReset(usernameInput: string | undefined, password: string) {
  ...
  if (policyError) throw new SqliteAuthError(400, policyError);
  const admins = db()
    .prepare("SELECT * FROM wfm_users WHERE is_admin = 1 ORDER BY username COLLATE NOCASE")
    .all() as UserRow[];
  const target = usernameInput
    ? admins.find((user) => user.username.toLowerCase() === usernameInput.toLowerCase())
    : admins.length === 1
      ? admins[0]
      : null;
  if (!target) {
    ...
    throw new SqliteAuthError(
      409,
      names ? `Specify an administrator username: ${names}` : "No administrator account exists.",
    );
  }
  const credential = newPassword(password);
  db()
