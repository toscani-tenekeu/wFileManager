...
const cleanPermissions = (value: unknown): string[] =>
  Array.isArray(value)
    ? [
        ...new Set(
          value.filter(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ownPermissions = auth.user.is_admin
      ? [...PERMISSIONS]
      : cleanPermissions(auth.role?.permissions);
    ...
    if (req.method === "GET") {
      if (
        !auth.user.is_admin &&
        !ownPermissions.includes("manage_roles") &&
        !ownPermissions.includes("manage_users")
      )
        return respond({ error: "Forbidden" }, 403);
      const [{ data: roles, error: rolesError }, { data: users, error: usersError }] =
        await Promise.all([
          db
            .from("wfilemanager_roles")
            .select("*")
      ...
      for (const user of users || [])
        if (user.role_id) counts.set(user.role_id, (counts.get(user.role_id) || 0) + 1);
      return respond({
        roles: (roles || []).map((role) => roleJson(role, counts.get(role.id) || 0)),
      });
    }
    ...
    if (req.method === "POST") {
      ...
      if (name.length < 2)
        return respond({ error: "Role name must contain at least 2 characters" }, 400);
