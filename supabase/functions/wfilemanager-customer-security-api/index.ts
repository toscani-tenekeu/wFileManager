...
function randomHex(length = 32) { const bytes = new Uint8Array(length); crypto.getRandomValues(bytes); return hex(bytes); }
function hexBytes(value: string) { const bytes = new Uint8Array(value.length / 2); for (let index = 0; index < bytes.length; index += 1) bytes[index] = Number.parseInt(value.slice(index * 2, index * 2 + 2), 16); return bytes; }
async function sha256(value: string) { return hex(new Uint8Array(await crypto.subtle.digest("SHA-256", encoder.encode(value)))); }
async function passwordHash(password: string, salt = randomHex(16), iterations = PASSWORD_ITERATIONS) {
  const key = await crypto.subtle.importKey("raw", encoder.encode(password), "PBKDF2", false, ["deriveBits"]);
  const bits = await crypto.subtle.deriveBits({ name: "PBKDF2", salt: hexBytes(salt), iterations, hash: "SHA-256" }, key, 256);
...
function clientIp(request: Request) { return (request.headers.get("x-forwarded-for") || request.headers.get("cf-connecting-ip") || "").split(",")[0].trim(); }
function bearer(request: Request) { return (request.headers.get("authorization") || "").match(/^Bearer\s+(.+)$/i)?.[1]?.trim() || ""; }
function escapeHtml(value: unknown) { return clean(value).replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[character]!); }

async function config() {
  const { data, error } = await db.from("wfilemanager_pro_subscription_config")
    .select("mailtrap_api_token,mailtrap_api_url,mailtrap_from_email,mailtrap_from_name,support_email,site_url")
...
async function sendMail(settings: Awaited<ReturnType<typeof config>>, params: { email: string; name: string; subject: string; text: string; html: string; category: string }) {
  ...
  const response = await fetch(settings.mailtrapUrl, {
    method: "POST",
    headers: { Authorization: `Bearer ${settings.mailtrapToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ from: { email: settings.fromEmail, name: settings.fromName }, to: [{ email: params.email, name: params.name || "Customer" }], subject: params.subject, text: params.text, html: params.html, category: params.category }),
  });
...
async function rateCheck(scope: string, identifier: string, ipAddress: string) {
  const { data, error } = await db.rpc("wfilemanager_auth_rate_check", { p_scope: scope, p_identifier_hash: await sha256(identifier), p_ip_address: ipAddress });
  if (error) throw error;
  return data as { allowed?: boolean; retryAfterSeconds?: number };
}
async function rateRecord(scope: string, identifier: string, ipAddress: string, success: boolean, limit = 5) {
  const { error } = await db.rpc("wfilemanager_auth_rate_record", { p_scope: scope, p_identifier_hash: await sha256(identifier), p_ip_address: ipAddress, p_success: success, p_limit: limit, p_window_minutes: 15, p_block_minutes: 15 });
...
async function authenticate(request: Request) {
  ...
  const { data, error } = await db.from("wfilemanager_customer_sessions")
    .select("id,customer_id,wfilemanager_customer_accounts(*)")
    .eq("token_hash", await sha256(token)).is("revoked_at", null)
...
async function issueToken(customer: Customer, purpose: "password_reset" | "email_verification", lifetimeMinutes: number) {
  const token = `wfm_${randomHex(32)}`;
  await db.from("wfilemanager_customer_auth_tokens").update({ consumed_at: new Date().toISOString() })
    .eq("customer_id", customer.id).eq("purpose", purpose).is("consumed_at", null);
  ...
  if (error) throw error;
  await db.from("wfilemanager_customer_auth_tokens").delete().lt("expires_at", new Date(Date.now() - 7 * 86400000).toISOString());
  return token;
}

async function requestPasswordReset(request: Request, body: Record<string, unknown>) {
  const email = normalizeEmail(body.email);
  if (!email) return json({ success: true, message: "If an account exists, a password reset email will be sent." });
  const ip = clientIp(request);
  const rate = await rateCheck("customer_password_reset", email, ip);
  if (rate.allowed === false) return json({ error: "Too many password reset requests. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
  const { data: customer, error } = await db.from("wfilemanager_customer_accounts").select("id,email,full_name,status").eq("email", email).maybeSingle();
  if (error) throw error;
  ...
  return json({ success: true, message: "If an account exists, a password reset email will be sent." });
}
...
async function resetPassword(request: Request, body: Record<string, unknown>) {
  ...
  if (rate.allowed === false) return json({ error: "Too many reset attempts. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
  ...
  if (error) {
    ...
    if (String(error.message).includes("invalid_or_expired_token")) return json({ error: "This password reset link is invalid or expired" }, 400);
    throw error;
  }
...
async function resendVerification(request: Request) {
  ...
  if (rate.allowed === false) return json({ error: "Too many verification requests. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
  const settings = await config();
...
async function verifyEmail(request: Request, body: Record<string, unknown>) {
  ...
  if (rate.allowed === false) return json({ error: "Too many verification attempts. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
  const { data, error } = await db.rpc("wfilemanager_consume_customer_auth_token", {
    p_token_hash: await sha256(token),
  ...
  if (error) {
    ...
    if (String(error.message).includes("invalid_or_expired_token")) return json({ error: "This verification link is invalid or expired" }, 400);
    throw error;
  }
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "request-password-reset" && request.method === "POST") return requestPasswordReset(request, await request.json().catch(() => ({})));
    if (action === "reset-password" && request.method === "POST") return resetPassword(request, await request.json().catch(() => ({})));
    if (action === "resend-verification" && request.method === "POST") return resendVerification(request);
    if (action === "verify-email" && request.method === "POST") return verifyEmail(request, await request.json().catch(() => ({})));
    if (action === "status") return json({ ok: true, passwordReset: true, emailVerification: true, tokenStorage: "hashed" });
    return json({ error: "Not found" }, 404);
  } catch (error) {
    console.error(error);
    return json({ error: error instanceof Error ? error.message : "Customer security service failed" }, 500);
  }
});
